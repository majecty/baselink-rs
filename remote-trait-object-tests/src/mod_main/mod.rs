...
fn start_server(with_cmd: ConnectionEnd, with_ping: ConnectionEnd) {
    ...
    let cmd_port = {
        ...
        Port::new(to_cmd, from_cmd, move |msg| {
            if msg == "start" {
                let ping_port = cloned_ctx.ping_port.lock().unwrap();
    ...
    let ping_port = {
        ...
        Port::new(to_ping, from_ping, |msg| {
            panic!(
                "main do not expect receiving packet frim ping. msg: {}",
                msg
            );
        })
    };

    *ctx.cmd_port.lock().unwrap() = Some(cmd_port);
    *ctx.ping_port.lock().unwrap() = Some(ping_port);
}
